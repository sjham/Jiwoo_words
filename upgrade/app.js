...
const GRADES={all:'전체',words:'단어 전체',1:'1단계',2:'2단계',3:'3단계',4:'숙어·표현',custom:'내 카드'};
...
function filtered(){return all().filter(c=>(state.prefs.grade==='all'||(state.prefs.grade==='words'&&c.g!==4)||(state.prefs.grade==='custom'?state.custom.some(x=>x.id===c.id):String(c.g)===state.prefs.grade)));}
...
function tags(c){return `<div class="badges"><span class="badge">${esc(GRADES[c.g]||'내 카드')}</span><span class="badge">${esc(pos(c))}</span>${status(c.id)!=='new'?`<span class="badge ${status(c.id)==='done'?'good':'bad'}">${status(c.id)==='done'?'완성함':'복습 중'}</span>`:''}</div>`;}
...
function speakButton(c,sentence=false){return `<button class="speaker ${sentence?'sentence-speaker':''}" data-speak="${esc(c.id)}" data-sentence="${sentence}" aria-label="${sentence?'예문':esc(c.en)} 듣기">${speakerIcon}${sentence?'<span>예문 듣기</span>':''}</button>`;}
...
function heading(kicker,title,description=''){return `<div class="intro"><div><h1>${esc(title)}</h1>${description?`<p>${esc(description)}</p>`:''}</div></div>`;}
function filterControls(){return `<details class="filters"><summary>학습 설정 · ${esc(GRADES[state.prefs.grade])}</summary><div class="filter-grid"><label>단계<select data-pref="grade">${options(GRADES,state.prefs.grade)}</select></label><label>출제 순서<select data-pref="order">${options({daily:'오늘 복습 + 새 단어',new:'새 단어만',repeat:'반복함만',done:'완성함 복습',star:'북마크만',all:'전체'},state.prefs.order)}</select></label><label>한 묶음<select data-pref="setSize">${options({5:'5장',10:'10장',20:'20장'},state.prefs.setSize)}</select></label><label>카드 방향<select data-pref="dir">${options({'en-ko':'영어 → 뜻','ko-en':'뜻 → 영어'},state.prefs.dir)}</select></label></div><p class="setting-help">단계는 난도별 묶음이며 특정 교과서의 수록 목록은 아닙니다. 변경한 범위는 새 묶음부터 적용됩니다.</p><button class="button small" data-action="new-study">이 설정으로 새 묶음</button></details>`;}
function studyChoice(){const g=state.prefs.grade;return `<div class="study-choice" role="group" aria-label="학습 종류">${[['all','전체'],['words','단어'],['4','숙어·표현']].map(([k,label])=>`<button data-study-kind="${k}" aria-pressed="${g===k||(k==='words'&&['1','2','3'].includes(g))}">${label}</button>`).join('')}</div>`;}
function studyView(){
 const t=todayRecord(),s=state.session;let card;
 if(!s){card=`<div class="note-card complete"><h2>${state.prefs.grade==='4'?'숙어·표현':state.prefs.grade==='words'?'단어':'오늘의 학습'}</h2><p>${filtered().length.toLocaleString()}장 중, 한 번에 ${state.prefs.setSize}장씩.</p><button class="button primary" data-action="start-study">${state.prefs.setSize}장 시작하기</button></div>`;}
 else if(!s.queue.length){card=`<div class="note-card empty"><h2>이 범위는 다 봤어요.</h2><p>다른 단계나 출제 순서를 골라 새 묶음을 시작해 보세요.</p></div>`;}
 else if(s.idx>=s.queue.length){card=`<div class="note-card complete"><h2>${s.initial}장 묶음, 끝.</h2><p>기억함 ${s.good}회 · 다시 보기 ${s.retry}회<br>${s.stampEarned?'오늘의 노트에 스탬프를 남겼어요.':'건너뛴 카드는 다음에 다시 볼 수 있어요.'}</p><div class="action-row"><button class="button primary" data-action="start-study">다음 ${state.prefs.setSize}장</button><button class="button" data-tab="quiz">다르게 플레이</button></div><button class="text-link" data-action="undo" ${!transientUndo?'disabled':''}>방금 분류 되돌리기</button></div>`;}
...
function recordsView(){
 ...
 return heading('','기록·선물')+`<section class="section-card"><h2>아빠와의 약속</h2><p class="muted small">첫 노트의 1,001장을 완성함에 모으면 선물이 열려요. 한 번 열린 선물은 그대로 유지됩니다.</p><div class="reward-list">${REWARDS.map(r=>{const need=Math.ceil(c.total*r.at),on=state.unlocked.includes(r.id)||c.done>=need;return `<div class="reward ${on?'unlocked':''}"><small>${Math.round(r.at*100)}% · ${on?'달성':'진행 중'}</small><strong>${r.name}</strong>${miniProgress(c.done,need)}<small>${on?'약속한 선물을 아빠와 확인해 봐.':`${need.toLocaleString()}장 중 ${c.done}장 · ${Math.max(0,need-c.done)}장 남음`}</small></div>`;}).join('')}</div><p class="setting-help">선물은 기존 ${c.total.toLocaleString()}장 기준 그대로. 추가 509장은 별도 확장 학습이며, 이미 열린 선물은 유지됩니다.</p></section><div class="summary-grid"><div class="stat"><span>이번 주 학습</span><strong>${w.cards}<small> 장</small></strong></div><div class="stat"><span>이번 주 포인트</span><strong>${w.points}<small> P</small></strong></div><div class="stat"><span>완주 스탬프</span><strong>${state.stamps.length}<small> 개</small></strong></div></div><section class="section-card"><h2>최근 7일</h2><div class="week">${Array.from({length:7},(_,i)=>{const k=shift(day(),i-6),r=state.daily[k],hit=r&&r.n>=r.goal;return `<div class="day ${hit?'hit':''} ${i===6?'today':''}"><small>${'일월화수목금토'[new Date(k+'T12:00:00').getDay()]}</small><strong>${r?.n||0}</strong><small>${hit?'달성':'장'}</small></div>`;}).join('')}</div><p class="muted small">하루 한 번, 학습 묶음이나 플레이를 끝내면 완주 스탬프 1개. ${state.stamps.length?`최근 완주: ${esc(state.stamps.at(-1))}`:'오늘의 첫 스탬프를 남겨 봐.'}</p><div class="stamp-line">${state.stamps.slice(-7).map(k=>`<span class="stamp-small">${esc(k.slice(5))} · Played</span>`).join('')}</div></section><section class="section-card"><h2>용돈 포인트</h2><p>아직 받지 않은 포인트 <strong>${balance()}P · ${money(balance())}</strong></p><p class="muted small">오늘 처음 학습한 카드마다 0.5P · 하루 목표 달성 +10P · 한 주 5일 달성 +20P.<br>카드 포인트는 하루 목표의 두 배까지. 같은 카드를 같은 날 다시 풀어도 중복 적립되지 않아요.</p><details class="subtle"><summary>아빠와 함께 확인</summary><p class="setting-help">자동 결제 기능은 없으며, 실제 지급한 용돈을 기록합니다.</p><button class="button" data-action="payout" ${balance()<=0?'disabled':''}>용돈 지급 완료 기록</button><div class="storage-info">${state.paid.slice().sort((a,b)=>b.ts-a.ts).slice(0,5).map(p=>`${new Date(p.ts).toLocaleDateString('ko-KR')} · ${p.p}P · ${Math.round(p.p*p.rate).toLocaleString()}원 지급`).join('<br>')}</div></details></section><details class="section-card"><summary>주별 기록</summary><div class="table-wrap"><table><thead><tr><th>시작일</th><th>학습</th><th>달성</th><th>포인트</th></tr></thead><tbody>${Object.entries(weeks()).sort(([a],[b])=>b.localeCompare(a)).map(([k,x])=>`<tr><td>${k}</td><td>${x.cards}장</td><td>${x.hits}일</td><td>${x.points}P</td></tr>`).join('')||'<tr><td colspan="4">첫 학습 뒤 기록이 나타납니다.</td></tr>'}</tbody></table></div></details>`;
}
...
function startGame(type){
 if(blocked)return;
 const base=state.prefs.grade==='custom'?filtered():type==='cloze'?all().filter(c=>c.g===4):state.prefs.grade==='4'?all().filter(c=>c.g!==4):filtered();
 let pool=base.filter(c=>type==='cloze'?/\{[^}]+\}/.test(c.ex):c.g!==4);
 ...
 if(list.length<(type==='match'?2:1)){toast('이 범위에는 문제가 부족해요. 다른 단계를 골라 주세요.');return;}
...
function quizView(){
 const g=state.game;
 if(!g)return heading('','플레이','짧게 한 판.')+`<div class="mode-grid"><button class="mode-card" data-game="spell"><span class="number">01</span><strong>철자 맞히기</strong><p>뜻과 첫 글자를 보고 영어를 직접 입력해 보기.</p></button><button class="mode-card" data-game="cloze"><span class="number">02</span><strong>숙어 빈칸 채우기</strong><p>표현을 문장에 맞게 바꾸어 빈칸 채우기.</p></button><button class="mode-card" data-game="match"><span class="number">03</span><strong>짝 맞추기</strong><p>영어와 뜻 연결하기. 틀리면 힌트를 보고 다시.</p></button></div><details class="section-card game-options"><summary>게임 범위 설정</summary><div class="filter-grid"><label>단계<select data-pref="grade">${options(GRADES,state.prefs.grade)}</select></label></div><p class="setting-help">철자·짝 맞추기는 단어로, 빈칸 채우기는 숙어로 플레이해요.</p></details><p class="preview-note">정답을 맞힌 카드는 복습 대상으로 남습니다. 다른 날에도 기억하면 완성함으로 이동해요.</p>`;
 const name={spell:'철자 맞히기',cloze:'숙어 빈칸 채우기',match:'짝 맞추기'}[g.type];
 let inner;
 if(g.complete){inner=`<div class="note-card complete"><h2>오늘의 한 판, 완료.</h2><p>${g.type==='match'?`${g.list.length}쌍을 모두 찾았어.`:`도움 없이 ${g.correct} / ${g.list.length}개 · 힌트로 ${g.assisted}개`}<br>헷갈렸던 표현은 복습할 목록에 남겨 뒀어.</p>${g.type!=='match'?`<div class="storage-info">${g.results.map(r=>`${r.passed?'기억함':'다시 보기'} · ${esc(index.get(r.id).en)}`).join('<br>')}</div>`:''}<div class="action-row" style="margin-top:22px"><button class="button primary" data-game="${g.type}">한 판 더</button><button class="button" data-action="game-menu">다른 플레이</button></div></div>`;}
...
function libraryView(){
 const q=library.q.toLowerCase().trim();let found=all().filter(c=>(!q||[c.en,c.ko,c.ex,c.kr].some(s=>s.toLowerCase().includes(q)))&&(library.grade==='all'||(library.grade==='words'&&c.g!==4)||(library.grade==='custom'?state.custom.some(x=>x.id===c.id):String(c.g)===library.grade))&&(library.status==='all'||(library.status==='star'?state.starred.includes(c.id):library.status==='due'?isDue(c):status(c.id)===library.status)));
 ...
 return heading('WORD ARCHIVE','나의 단어장',`단어 ${all().filter(c=>c.g!==4).length.toLocaleString()}개 · 숙어와 표현 ${all().filter(c=>c.g===4).length}개. 펼치면 예문과 해석까지.`)+`<div class="searchbar"><label>검색<input id="library-search" type="search" enterkeyhint="search" placeholder="단어·뜻·예문 검색" value="${esc(library.q)}"></label><label>모아 보기<select data-filter="status">${options({all:'전체',new:'새 단어',repeat:'반복함',done:'완성함',star:'북마크',due:'오늘 복습'},library.status)}</select></label><label>단계<select data-filter="grade">${options(GRADES,library.grade)}</select></label></div><p class="muted small" style="margin-bottom:12px">${found.length.toLocaleString()}개 검색됨 · ${library.page+1} / ${pages}페이지</p><div class="library-list">${items.map(c=>`<details class="library-item"><summary><strong lang="en">${esc(c.en)}</strong><span class="gloss">${esc(c.ko)}</span><span class="badge ${status(c.id)==='done'?'good':status(c.id)==='repeat'?'bad':''}">${status(c.id)==='new'?'새 단어':status(c.id)==='done'?'완성함':'반복함'} <span class="expand-cue" aria-hidden="true">⌄</span></span></summary><div class="library-detail"><div class="library-audio">${speakButton(c)}<span>${c.g===4?'숙어':'단어'} 듣기</span></div><p class="muted small" style="padding-top:12px">${esc(pos(c))}${state.reviews[c.id]?` · 다음 복습 ${esc(state.reviews[c.id].due)}`:''}</p>${example(c)}<div class="action-row"><button class="button small bookmark" data-star="${c.id}" aria-pressed="${state.starred.includes(c.id)}">${state.starred.includes(c.id)?'북마크됨':'북마크'}</button><button class="button small" data-open-card="${c.id}">이 카드 학습</button>${state.custom.some(x=>x.id===c.id)?`<button class="button small danger" data-delete="${c.id}">내 카드 삭제</button>`:''}</div></div></details>`).join('')||'<div class="empty">검색 결과가 없어요. 다른 단어나 범위를 골라 보세요.</div>'}</div><div class="pagination"><button class="button small" data-page="-1" ${library.page===0?'disabled':''}>이전</button><span class="small muted">${library.page+1} / ${pages}</span><button class="button small" data-page="1" ${library.page+1>=pages?'disabled':''}>다음</button></div><details class="section-card"><summary>내 카드 추가</summary><form id="add-card" class="form-grid" style="margin-top:18px"><label>영어<input name="en" required maxlength="150" placeholder="예: borrow"></label><label>뜻<input name="ko" required maxlength="500" placeholder="예: 빌리다"></label><label>품사<select name="pos">${options({'':'선택',n:'명사',v:'동사',adj:'형용사',adv:'부사',prep:'전치사',phr:'표현'},'')}</select></label><label>단계<select name="g">${options({0:'내 카드',1:'1단계',2:'2단계',3:'3단계',4:'숙어·표현'},0)}</select></label><label class="full">예문<input name="ex" maxlength="1500" placeholder="문장 빈칸: Please {turn off} the light."></label><label class="full">해석<input name="kr" maxlength="1500" placeholder="불을 꺼 주세요."></label><div class="full"><button class="button primary">카드 추가</button></div></form></details><details class="section-card"><summary>여러 장 한꺼번에 추가</summary><p class="setting-help">한 줄에 ‘단어, 뜻, 예문’. 엑셀에서 복사한 탭 구분도 가능합니다. 뜻에 쉼표가 들어가면 탭으로 구분하세요.</p><form id="bulk-add"><label>붙여넣기<textarea name="bulk" maxlength="200000" placeholder="borrow, 빌리다, Can I borrow your pen?"></textarea></label><button class="button" style="margin-top:12px">추가하기</button></form></details>`;
}
function settingsView(){return heading('','설정')+`<section class="section-card"><h2>학습과 화면</h2><div class="form-grid"><label>하루 목표<select data-pref="goal">${options({5:'5장',10:'10장',15:'15장',20:'20장',30:'30장',40:'40장'},state.prefs.goal)}</select></label><label>화면<select data-pref="theme">${options({auto:'기기 설정 따르기',light:'밝은 종이',dark:'밤의 노트'},state.prefs.theme)}</select></label><label>발음 속도<select data-pref="speechRate">${options({'0.65':'천천히','0.85':'편안하게',1:'보통'},state.prefs.speechRate)}</select></label><label class="check"><input type="checkbox" data-pref="translate" ${state.prefs.translate?'checked':''}>해석을 바로 표시</label><label class="check"><input type="checkbox" data-pref="sound" ${state.prefs.sound?'checked':''}>학습 완료 효과음</label></div><p class="setting-help">이미 공부한 날의 목표는 유지되고, 바꾼 목표는 다음 학습일에 적용돼요. 발음은 기기에서 제공하는 영어 음성입니다.</p></section><details class="section-card"><summary>아빠와 함께 설정</summary><label>1P의 용돈<input type="number" id="point-rate" min="0" max="100000" step="1" value="${state.prefs.rate}"></label></details><section class="section-card"><h2>백업과 기기 이동</h2><p class="storage-info">진도는 이 기기의 브라우저에만 저장됩니다. 기기를 바꾸기 전에는 백업 파일을 받아 새 기기에서 불러오세요. 브라우저 데이터 삭제 시 기록이 사라질 수 있습니다.</p><div class="action-row"><button class="button primary" data-action="backup">백업 파일 받기</button><label class="button">백업 불러오기<input type="file" id="restore-file" accept=".json,application/json" hidden></label></div><p class="setting-help">불러오기는 현재 미리보기 기록을 교체합니다. 먼저 현재 기록을 백업해 두세요.</p><details class="subtle"><summary>기록 복구 도움말</summary><p class="storage-info">백업이 정상적으로 열리지 않을 때 원본 기록을 보관할 수 있어요.</p><button class="button" data-action="raw-backup">저장 원본 받기</button></details><details class="subtle"><summary>기존 지우 앱의 진도</summary><p class="storage-info">처음 미리보기를 열면 같은 브라우저에 있는 기존판 진도를 복사합니다. 이후 두 앱은 따로 저장되며, 기존판 기록은 수정하지 않습니다.</p><button class="button" data-action="legacy-import">기존판 진도 다시 가져오기</button><p class="setting-help">이 버튼은 현재 미리보기 진도를 기존판 기록으로 교체합니다.</p></details></section><details class="section-card"><summary>아빠의 편지</summary><div class="letter">지우야.<br>아빠가 지우를 위해 만들어봤다.<br>공부한다고 생각하지 말고<br>즐겁게 가지고 놀아.<br><br>— 아빠 —</div></details><details class="section-card"><summary>이 노트의 사용법</summary><p class="storage-info">‘기억했어’는 완성함으로, ‘다시 볼래’는 반복함으로 보냅니다. 반복함 카드는 같은 묶음에서 한 번 더 등장합니다. 잘 기억한 날이 쌓이면 복습 간격이 1·3·7·14·30·60일로 늘어납니다. 같은 날 반복해서 맞혀도 간격은 늘어나지 않습니다.</p><p class="storage-info">플레이에서 처음 맞힌 단어는 다음 날 다시 확인합니다. 직접 학습 카드에서 ‘기억했어’를 눌러 완성함으로 보낼 수도 있습니다.</p><p class="storage-info">단어 1,200개 · 숙어·생활 표현 310개. 단계는 학습 편의를 위한 묶음입니다. 표현 메모에는 자주 쓰는 말과 헷갈리기 쉬운 문법·발음 구분을 정리했습니다.</p><p class="storage-info">홈 화면 추가: 아이폰은 사파리의 공유 → 홈 화면에 추가. 갤럭시는 크롬 메뉴의 설치 또는 홈 화면에 추가. 미리보기는 기존판과 별도로 추가할 수 있습니다.</p></details><details class="section-card"><summary>미리보기 진도 초기화</summary><p class="setting-help">현재 미리보기의 분류·복습 일정·진행 중 묶음을 초기화합니다. 내 카드, 학습 일별 기록, 지급 내역, 열린 선물은 유지합니다.</p><button class="button danger" data-action="reset">분류와 복습 일정 초기화</button></details>`;}
...
document.addEventListener('click',e=>{
 ...
 if(b.dataset.studyKind){if(blocked||state.prefs.grade===b.dataset.studyKind)return;if(state.session&&state.session.idx<state.session.queue.length&&!confirm('학습 종류를 바꿀까요? 지금까지의 학습 기록은 남습니다.'))return;state.prefs.grade=b.dataset.studyKind;state.session=null;transientUndo=null;save();render();return;}
 ...
 if(['start-study','new-study','challenge'].includes(a)){if(state.session&&state.session.idx<state.session.queue.length&&!confirm('새 묶음으로 바꿀까요? 지금까지 공부한 기록은 남습니다.'))return;startStudy(a==='challenge'?5:state.prefs.setSize,a==='challenge'?'5장 챌린지':'오늘의 학습');}
...
document.addEventListener('change',async e=>{
 const x=e.target;
 if(x.dataset.pref){const k=x.dataset.pref;let v=x.type==='checkbox'?x.checked:x.value;const opened=Array.from(document.querySelectorAll('details')).map(d=>d.open),y=scrollY;if(['goal','setSize','speechRate'].includes(k))v=Number(v);setPref(k,v);document.querySelectorAll('details').forEach((d,i)=>{d.open=opened[i]||false;});document.querySelector(`[data-pref="${k}"]`)?.focus({preventScroll:true});window.scrollTo(0,y);return;}
...
function keyboardLayout(){const editing=document.activeElement?.matches('input,textarea');document.body.classList.toggle('keyboard-open',!!(editing&&window.visualViewport&&innerHeight-visualViewport.height>120));}
window.visualViewport?.addEventListener('resize',keyboardLayout);document.addEventListener('focusin',keyboardLayout);document.addEventListener('focusout',()=>requestAnimationFrame(keyboardLayout));
