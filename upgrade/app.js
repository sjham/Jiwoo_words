...
function filtered(){return all().filter(c=>(state.prefs.grade==='all'||(state.prefs.grade==='custom'?state.custom.some(x=>x.id===c.id):String(c.g)===state.prefs.grade)));}
...
function studyView(){
 ...
 if(!s){card=`<div class="note-card complete"><h2>오늘의 ${state.prefs.setSize}장</h2><p>한 장씩, 가볍게 시작해.</p><button class="button primary" data-action="start-study">${state.prefs.setSize}장 시작하기</button></div>`;}
 ...
 return `<div class="study-layout"><div class="study-main"><div class="today-strip"><span>오늘 <strong>${t.n} / ${t.goal}장</strong></span><span class="muted">${t.n>=t.goal?'목표 달성':''}</span></div>${miniProgress(t.n,t.goal)}${card}${filterControls()}</div></div>`;
}
...
function quizView(){
 const g=state.game;
 if(!g)return heading('','플레이','짧게 한 판.')+`<div class="section-card"><div class="filter-grid"><label>단계<select data-pref="grade">${options(GRADES,state.prefs.grade)}</select></label></div><p class="setting-help">단계 설정은 학습 탭과 함께 사용합니다. 숙어 빈칸은 ‘전체 단계’ 또는 ‘숙어·표현’으로 선택하세요.</p></div><button class="button small play-challenge" data-action="challenge">5장 학습 챌린지</button><div class="mode-grid"><button class="mode-card" data-game="spell"><span class="number">01</span><strong>철자 맞히기</strong><p>뜻과 첫 글자를 보고 영어를 직접 입력해 보기.</p></button><button class="mode-card" data-game="cloze"><span class="number">02</span><strong>빈칸 채우기</strong><p>표현을 문장에 맞게 바꾸어 빈칸 채우기.</p></button><button class="mode-card" data-game="match"><span class="number">03</span><strong>짝 맞추기</strong><p>영어와 뜻 연결하기. 틀리면 힌트를 보고 다시.</p></button></div><p class="preview-note">정답을 맞힌 카드는 복습 대상으로 남습니다. 다른 날에도 기억하면 완성함으로 이동해요.</p>`;
