...
function quizView(){
 const g=state.game;
 if(!g)return heading('','플레이','짧게 한 판.')+`<div class="section-card"><div class="filter-grid"><label>단계<select data-pref="grade">${options(GRADES,state.prefs.grade)}</select></label></div><p class="setting-help">철자·짝 맞추기는 단어로, 빈칸 채우기는 숙어로 플레이해요.</p></div><div class="mode-grid"><button class="mode-card" data-game="spell"><span class="number">01</span><strong>철자 맞히기</strong><p>뜻과 첫 글자를 보고 영어를 직접 입력해 보기.</p></button><button class="mode-card" data-game="cloze"><span class="number">02</span><strong>숙어 빈칸 채우기</strong><p>표현을 문장에 맞게 바꾸어 빈칸 채우기.</p></button><button class="mode-card" data-game="match"><span class="number">03</span><strong>짝 맞추기</strong><p>영어와 뜻 연결하기. 틀리면 힌트를 보고 다시.</p></button></div><p class="preview-note">정답을 맞힌 카드는 복습 대상으로 남습니다. 다른 날에도 기억하면 완성함으로 이동해요.</p>`;
 const name={spell:'철자 맞히기',cloze:'빈칸 채우기',match:'짝 맞추기'}[g.type];
 let inner;
